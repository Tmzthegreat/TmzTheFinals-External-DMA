...
AimAssist::AimAssist() : Module(obf("AimAssist"), obf("Helps you at Aiming")) {
	...
	disss = &SetManager::i().add(new Set(500, 0, 1000, obf("Distence select"), getName())).getVec3i();
}
...
void AimAssist::renderImGui() {
	...
	ImGui::Text("Distence select");
	ImGui::SliderInt_("##DistenceSelect", &disss->x, disss->y, disss->z);
}