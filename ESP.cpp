...
ESP::ESP() : Module(obf("ESP"), obf("Render Entities through Walls"), VK_DIVIDE) {
	...
	colPlayername = &SetManager::i().add(new Set(ImGuiHelper::rgbaToVec4(114, 144, 222, 200), obf("colPlayername"), getName())).getVec4();
	colPlayerinfo = &SetManager::i().add(new Set(ImGuiHelper::rgbaToVec4(188, 169, 154, 200), obf("colPlayerinfo"), getName())).getVec4();

	isSkeleton = &SetManager::i().add(new Set(true, obf("isSkeleton"), getName())).getBVal();
	is2DBox = &SetManager::i().add(new Set(true, obf("is2DBox"), getName())).getBVal();
	isPlayername = &SetManager::i().add(new Set(true, obf("isPlayername"), getName())).getBVal();
	isPlayerinfo = &SetManager::i().add(new Set(true, obf("isPlayerinfo"), getName())).getBVal();
	isHealthBar = &SetManager::i().add(new Set(true, obf("isHealthBar"), getName())).getBVal();
...
void ESP::renderImGui() {
	...
	if (ImGui::BeginCombo(obf("PlayerName Style").c_str(), obf("Style").c_str(), 0)) {
		ImGui::Checkbox_(obf("PlayerName").c_str(), isPlayername);
		ImGui::ColorEdit4(obf("PlayerName Color##1").c_str(), (float*)colPlayername, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
		//ImGui::Checkbox_(obf("Skeleton Based on Health").c_str(), isSkeletonHealthBased);
		ImGui::EndCombo();
	}
	if (ImGui::BeginCombo(obf("PlayerInfo Style").c_str(), obf("Style").c_str(), 0)) {
		ImGui::Checkbox_(obf("PlayerInfo").c_str(), isPlayerinfo);
		ImGui::ColorEdit4(obf("PlayerInfo Color##1").c_str(), (float*)colPlayerinfo, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
		//ImGui::Checkbox_(obf("Skeleton Based on Health").c_str(), isSkeletonHealthBased);
		ImGui::EndCombo();
	}
